...
export function ContextNav() {
  ...
  const [expandedFolders, setExpandedFolders] = useState<Set<string>>(() => {
    ...
    const stored = localStorage.getItem(`sla_expanded_folders_${currentSection}`)
    if (stored) {
      try {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem(
        `sla_expanded_folders_${currentSection}`,
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      const nav = document.querySelector('[data-context-nav]')
      if (!nav?.contains(document.activeElement)) return

      if (event.key === "Enter" || event.key === " ") {
        const target = event.target as HTMLElement
        if (target.getAttribute("role") === "treeitem" && target.hasAttribute("data-folder-label")) {
          event.preventDefault()
          const folderLabel = target.getAttribute("data-folder-label")
          if (folderLabel) {
            setExpandedFolders((prev) => {
              const next = new Set(prev)
              if (next.has(folderLabel)) {
  ...
  return (
    <nav
      ...
      role="tree"
      className={cn(
        "fixed left-[72px] top-0 h-screen w-[280px] flex flex-col",
    ...
    >
      ...
      <div className="flex-shrink-0 p-4 border-b border-gray-200/60 dark:border-gray-800/60">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
          {currentRailSection?.label || "Navigation"}
        </h2>
      </div>
