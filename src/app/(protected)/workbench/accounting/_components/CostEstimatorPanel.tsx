...
export function CostEstimatorPanel({ 
  ...
  const [localInputs, setLocalInputs] = useState<CostEstimatorInputs>(externalInputs || costEstimatorInputs)
  const [localBreakdown, setLocalBreakdown] = useState<CostBreakdown>(externalBreakdown || mockCostBreakdown)

  const inputs = externalInputs || localInputs
  const breakdown = externalBreakdown || localBreakdown
